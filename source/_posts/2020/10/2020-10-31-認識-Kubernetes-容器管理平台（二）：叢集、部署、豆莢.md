...
Pod 為特定於應用程式的「邏輯主機」建模，並且可以包含相對高度耦合的不同應用容器。例如 Pod 可能既包含帶有 Node.js 應用的容器，也包含另一個不同的容器，用於提供 Node.js 網路伺服器要發布的資料。Pod 中的容器共享 IP 位址和埠，始終位於同一位置並且共同調度，並在同一個 Node 上的共享上下文中運行。

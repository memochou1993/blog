---
title: 使用範例開發 Solana 鏈上程式
permalink: 使用範例開發-Solana-鏈上程式
date: 2022-04-14 15:16:51
tags: ["區塊鏈", "Rust", "Solana", "Web3", "JavaScript", "Node", "Smart Contract", "DApp"]
categories: ["區塊鏈"]
---
...
使用 `solana-keygen` 指令建立一組公私鑰。

```BASH
solana-keygen new
```

將命令列設定的 `url` 參數設置成 `localhost` 叢集。

```BASH
solana config set --url localhost
```

開啟一個新的終端視窗，執行 `solana-test-validator` 指令，啟動一個本地的 Solana 叢集。
